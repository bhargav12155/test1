...
Main.KiteMoverInstr.prototype.changeText = function () {
    audio.pause();
    playSound('sounds/never_fly_kite.mp3');// adding the sound function
...
Main.KiteMover.prototype.setPlayer = function ()
{
    ...
    player.width = 100;//120
    player.height = 150;//150
...
Main.KiteMover.prototype.setObstacles = function () {
    var obst = obstacles.create(250, -3, 'wire_long');//y 0
    //obst.height = 300;
    //obst.width = 20;

    obst = obstacles.create(250, 600, 'wire_long');
    //obst.height = 250;
    //obst.width = 20;

    obst = obstacles.create(530, 300, 'wire_med');
    //obst.height = 220;
    //obst.width = 20;

    obst = obstacles.create(830, -3, 'wire_med');
    //obst.height = 200;
    //obst.width = 20;

    obst = obstacles.create(830, 500, 'wire_long');
    //obst.height = 250;
    //obst.width = 20;
}
...
Main.KiteMover.prototype.setEmitter = function ()
{
    ...
    //emitter = this.game.add.emitter(0, 0, 100);
    //makeElectricEmitter(emitter);
}

Main.KiteMover.prototype.setText = function () {
    audio.pause();
    playSound('sounds/move_kite.mp3');// adding the sound function
...
Main.TubMoverInstr.prototype.changeText = function () {
    audio.pause();
    playSound('sounds/water_is_really.mp3');// adding the sound function
...
Main.TubMover.prototype.setObjects = function () {
    audio.pause();
    playSound('sounds/move_bath.mp3');// adding the sound function
    applianceStringArray = ["falling_lamp", "falling_toaster", "falling_phone", "falling_controller"];//["lamp", "toaster", "littlephone", "controller", "littleoutlet"];
...
Main.WetHandsInstr.prototype.changeText = function () {
    audio.pause();
    playSound('sounds/just_having.mp3');// adding the sound function
...
Main.WetHands.prototype.setPlayer = function ()
{
    ...
    player = this.game.add.sprite(this.game.world.centerX, this.game.world.centerY, playerString);//'hands'
...
Main.WetHands.prototype.setObstacles = function () {
    obstaclesList = ["falling_lamp", "falling_toaster", "falling_phone", "falling_controller"];//['lamp', 'toaster', 'controller', 'littlephone'];
...
Main.WetHands.prototype.setText = function () {
    audio.pause();
    playSound('sounds/keep_wet_hands.mp3');// adding the sound function
...
Main.LadderInstr.prototype.changeText = function () {
    audio.pause();
    playSound('sounds/you_should_keep.mp3');// adding the sound function
...
{
    //setBackground('background_ladder');
    if (typeof background != 'undefined')
        background.loadTexture('background_ladder');

    //power line
    danger = this.game.add.sprite(230, this.game.world.centerY, 'powerLine');//170
    danger.anchor.setTo(0.5, 0.5);

    //safety line
    safety = this.game.add.graphics();
    safety.beginFill(0x000000);
    safety.lineStyle(10, 0x000000, 1);
    safety.moveTo(danger.x + 400, 0);
    ...
    audio.pause();
    playSound('sounds/move_ladder.mp3');// adding the sound function
    //text
    text = "Move the ladder behind the safety line!";
}
...
{
    playSound('sounds/wall_outlets.mp3');// adding the sound function
...
{
    audio.pause();
    playSound('sounds/which_outlets.mp3');// adding the sound function
...
Main.PullPlugInstr.prototype.changeText = function () {
    audio.pause();
    playSound('sounds/the_best_way.mp3');// adding the sound function
...
Main.PullPlug.prototype.setText = function () {
    audio.pause();
    playSound('sounds/guide.mp3');// adding the sound function
...
Main.MoveFurnitureInstr.prototype.changeText = function () {
    audio.pause();
    playSound('sounds/you_shouldnt.mp3');// adding the sound function
...
Main.MoveFurniture.prototype.setUpEvacArea = function (){
    ...
    audio.pause();
    playSound('sounds/move_the_furn.mp3');// adding the sound function
