...
Main.ClickAll.prototype = {
    ...
    update: function () {
        globalUpdate();
        var count = 0;
        if (current == appliances.children.length && trialOver == false)
            win();
        if (timeUp)
            this.lose();
    },
...
Main.Collect.prototype = {
        ...
        for (var i = 0; i < collectionObjectsArray.length + wrongObjectsArray.length; i++) {
            ...
            switch (quadrant) {
                case 0: //left quadrant
                    x = this.game.rnd.integerInRange(0 + widthBuffer, this.game.width / 2 - goal.height / 2);
                ...
                case 1://top quadrant
                    ...
                    y = this.game.rnd.integerInRange(0 + widthBuffer, this.game.height / 2 - goal.height / 2);
                    break;
                case 2: //right quadrant
                    x = this.game.rnd.integerInRange(this.game.width / 2 + goal.height / 2, this.game.width - widthBuffer);
                    y = this.game.rnd.integerInRange(0 + widthBuffer, this.game.height - widthBuffer);
                    break;
                case 3: //bottom quadrant
                    x = this.game.rnd.integerInRange(0 + widthBuffer, this.game.width);
                    y = this.game.rnd.integerInRange(this.game.height / 2 + goal.height / 2, this.game.height - widthBuffer);
                    break;
                default: x = y = 0;
            }//switch
    ...
    update: function () {
        globalUpdate();
        if (itemsInGoal >= collectionObjectsArray.length && trialOver == false)
            win();
        if (timeUp)
            this.lose();
        if (trialOver) {
...
Main.FallingObjects.prototype = {
    ...
    hitObstacle: function (obstacle)
    {
        ...
        if (usesEmitter)//typeof emitter != 'undefined' && emitter != null)
        {
            emitter.x = spriteLeft(obstacle) + obstacle.width/2;// + emitterPositionOffset.x;//collector
            emitter.y = obstacle.position.y;// + emitterPositionOffset.y;
            emitter.start(true, 2000, null, 16);
        }
    },
...
Main.DragToGoal.prototype = {
    ...
    update: function ()
    {
        ...
        if (started && trialOver == false) {
            this.otherUpdate();
            if (player.inputEnabled && player.input.draggable == false)
            ...
            if (testAABBCollision(player, goal) && !inside) {
                this.gameWon();
            } else if (spriteRight(player) -50 >= goal.x && inside) {//player.position.x >= goal.x
                this.gameWon();
            }
            if (timeUp) {
                this.lose();
